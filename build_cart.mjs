    ...
    <script>
      (function() {
        ...
        function doSearchGorlovkaAddresses(val) {
          ...
          if (activeNominatimAbort) {
            try { activeNominatimAbort.abort(); } catch(e) {}
          }
          var controller = typeof AbortController !== 'undefined' ? new AbortController() : null;
          activeNominatimAbort = controller;
          ...
          var nomUrl = 'https://nominatim.openstreetmap.org/search?q=' + encodeURIComponent('Горловка ' + cleanVal) + '&format=json&accept-language=ru&limit=10&viewbox=37.85,48.45,38.25,48.20';

          fetch(nomUrl, controller ? { signal: controller.signal } : {})
            .then(function(res) { return res.json(); })
            .then(function(items) {
              if (!items || items.length === 0) return;

              var nomList = items.map(function(it) {
                var rawName = it.display_name || '';
                var cleanTitle = rawName
                  .replace(/Донецкая Народная Республика[^,]*[,]?[ ]*/gi, '')
                  .replace(/ДНР[^,]*[,]?[ ]*/gi, '')
                  .replace(/Донецкая область[^,]*[,]?[ ]*/gi, '')
                  .replace(/Горловская городская община[^,]*[,]?[ ]*/gi, '')
                  .replace(/Горловский район[^,]*[,]?[ ]*/gi, '')
                  .replace(/Украина[^,]*/gi, '')
                  .replace(/[0-9]{5}[,]?[ ]*/g, '')
                  .replace(/г\\.?\\s*Горловка,\\s*/gi, '')
                  .replace(/Горловка,\\s*/gi, '')
                ...
                if (!cleanTitle) cleanTitle = rawName.split(',')[0];

                var coords = [parseFloat(it.lat), parseFloat(it.lon)];
                var zone = getZoneForCoords(coords);

                return {
                  title: cleanTitle,
                  full: 'Горловка, ' + cleanTitle,
                  dist: zone.dist,
                  price: zone.price,
                  time: zone.time,
                  coords: coords,
                ...
                return item.coords && !isNaN(item.coords[0]) && !isNaN(item.coords[1]);
              });

              // Merge local results with Nominatim live results
              var combined = local.slice();
              var seen = {};
              combined.forEach(function(c) { seen[c.title.toLowerCase()] = true; });

              nomList.forEach(function(nom) {
                var key = nom.title.toLowerCase();
                if (!seen[key]) {
                  seen[key] = true;
                  combined.push(nom);
                }
