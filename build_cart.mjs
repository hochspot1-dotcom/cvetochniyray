                ...
                <div class="co-method-content" id="co-courier-section">
                  <div class="co-address-preview-card">
                    <div class="co-address-card-info">
                      ...
                      <div class="co-address-title" id="co-selected-address-text" style="color: #6C8375;">Адрес доставки не выбран (выберите на карте или введите)</div>
                      <div class="co-address-meta" id="co-selected-zone-text" style="display: none;">
          ...
          <div class="checkout-summary-col">
            <div class="checkout-summary-card">
              ...
              <div class="co-totals-box">
                ...
                <div class="co-totals-row">
                  <span>Доставка:</span>
                  <span id="co-sum-delivery" style="font-weight: 700; color: #6C8375;">—</span>
                </div>
        ...
        <div class="address-modal-body delivery-modal-body-vertical">
          ...
          <div class="delivery-search-section">
            <div class="delivery-main-input-wrap">
              ...
              <input type="text" class="delivery-main-input" id="map-address-input" placeholder="Введите улицу и дом в Горловке (или кликните на карте)..." value="" autocomplete="off" />
            </div>
          ...
          <div class="delivery-modal-footer">
            ...
            <button class="btn-modal-confirm" id="btn-apply-address">
              <span id="btn-apply-text">Подтвердить адрес</span>
            </button>
          </div>
        </div>
      </div>
      ...
      (function() {
        ...
        try {
          ...
          if (savedAddr) {
            ...
            if (parsed && parsed.address && parsed.isAddressChosen) {
              deliveryInfo = parsed;
            }
          }
        } catch(e) {}
        ...
        function openAddressModal() {
          ...
          if (mapAddressInput) mapAddressInput.value = (deliveryInfo.isAddressChosen && deliveryInfo.address) ? deliveryInfo.address : '';
          updateModalCalcView();
        ...
        function updateModalCalcView() {
          var isChosen = deliveryInfo.isAddressChosen && deliveryInfo.address;
          var priceVal = deliveryInfo.price || 250;
          var priceStr = isChosen ? (deliveryInfo.price === 0 ? 'Бесплатно (0 ₽)' : deliveryInfo.price + ' ₽') : '—';
          if (calcPriceText) calcPriceText.textContent = priceStr;
          if (calcTimeText) calcTimeText.textContent = isChosen ? deliveryInfo.time : '30-45 мин';
          if (calcZoneName) calcZoneName.textContent = isChosen ? (deliveryInfo.district + ' (' + priceStr + ')') : 'Укажите адрес на карте или в поиске';
          if (clientPinTooltip) clientPinTooltip.textContent = isChosen ? (deliveryInfo.address + ' (' + priceStr + ')') : 'Кликните на дом на карте';
          var btnApplyText = document.getElementById('btn-apply-text');
          if (btnApplyText) {
            btnApplyText.textContent = isChosen ? ('Подтвердить адрес (' + priceVal + ' ₽)') : 'Подтвердить адрес';
          }
        }
        ...
        var GORLOVKA_STREET_REGISTRY = [
          // Центрально-Городской район (ЦГР, 250 ₽)
          ...
          { street: 'ул. Комсомольская', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3040, 38.0150] },
          { street: 'ул. Герцена', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3070, 38.0130] },
          { street: 'ул. Декабристов', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3080, 38.0120] },
          { street: 'ул. Семашко', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3060, 38.0110] },
          { street: 'ул. Ухтомского', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3050, 38.0100] },
          { street: 'ул. Мичурина', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3090, 38.0105] },
          { street: 'ул. Челюскинцев', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3030, 38.0115] },
          { street: 'ул. Радищева', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3020, 38.0125] },
          { street: 'ул. Патона', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3010, 38.0135] },
          { street: 'ул. Пионерская', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3045, 38.0200] },
          { street: 'ул. Школьная', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3035, 38.0225] },
          { street: 'ул. Больничная', dist: 'ЦГР', price: 250, time: '30 мин', coords: [48.3055, 38.0245] },
          ...
          { street: 'ул. Гречко', dist: 'Короленко', price: 300, time: '30 мин', coords: [48.2935, 38.0385] },
          { street: 'ул. Грибоедова', dist: 'Короленко', price: 300, time: '30 мин', coords: [48.2915, 38.0365] },
          ...
          // Северная проходная и ж/д станция (300 ₽)
          ...
          { street: 'ул. Вокзальная', dist: 'Северная проходная', price: 300, time: '40 мин', coords: [48.3210, 38.0250] },

          // 88-й квартал и ж/м Солнечный (330 ₽)
          ...
          { street: 'ж/м Солнечный', dist: '88-й квартал', price: 330, time: '40 мин', coords: [48.3170, 38.0490] },
          { street: 'ул. Пражская', dist: '88-й квартал', price: 330, time: '40 мин', coords: [48.3200, 38.0470] },
          { street: 'ул. Магистральная', dist: '88-й квартал', price: 330, time: '40 мин', coords: [48.3150, 38.0450] },
          ...
          { street: 'ул. Нахимова', dist: 'Кочегарка', price: 330, time: '40 мин', coords: [48.3130, 37.9940] },
          ...
          { street: 'ул. Добролюбова', dist: 'Майский', price: 330, time: '30 мин', coords: [48.2950, 38.0570] },
          ...
          { street: 'ул. Маршала Жукова', dist: 'Строитель', price: 350, time: '40 мин', coords: [48.3270, 37.9600] },
          { street: 'ул. Ленина (Строитель)', dist: 'Строитель', price: 350, time: '40 мин', coords: [48.3260, 37.9650] },
          { street: 'ул. Моисеенко', dist: 'Строитель', price: 350, time: '40 мин', coords: [48.3250, 37.9680] },
          { street: 'ул. Врубеля', dist: 'Строитель', price: 350, time: '40 мин', coords: [48.3275, 37.9630] },
          ...
          { street: 'ул. Корчагина', dist: '5-й квартал', price: 400, time: '40 мин', coords: [48.3315, 38.0370] },
          ...
          { street: 'ул. Фурманова', dist: 'Новогорловка', price: 400, time: '40 мин', coords: [48.3130, 38.0860] },
          ...
          { street: 'ул. Котовского', dist: 'Аксёновка', price: 400, time: '40 мин', coords: [48.2810, 38.0470] },
          ...
          { street: 'ул. Щербакова', dist: 'пос. Кирова', price: 450, time: '40 мин', coords: [48.2840, 37.9790] },
          ...
          { street: 'ул. Ушинского', dist: 'Пятая шахта', price: 450, time: '40 мин', coords: [48.2740, 38.0040] },
          ...
          { street: 'ул. Краснодонская', dist: 'Финский', price: 450, time: '50 мин', coords: [48.3370, 38.0640] },
          ...
          { street: 'ул. Бойко', dist: 'Румянцево', price: 470, time: '50 мин', coords: [48.3410, 38.0100] },
          ...
          { street: 'ул. Дружбы', dist: 'Мирный', price: 480, time: '40 мин', coords: [48.2640, 38.0610] },

          // Комсомолец (500 ₽)
          { street: 'пос. Комсомолец', dist: 'Комсомолец', price: 500, time: '45 мин', coords: [48.3450, 37.9350] },
          { street: 'ж/м Комсомолец', dist: 'Комсомолец', price: 500, time: '45 мин', coords: [48.3450, 37.9350] },
          ...
          { street: 'ул. Ртутная', dist: 'Комсомолец', price: 500, time: '45 мин', coords: [48.3470, 37.9380] },
          ...
          // Бессарабка (650 ₽)
          { street: 'пос. Бессарабка', dist: 'Северная проходная', price: 550, time: '50 мин', coords: [48.3400, 37.9850] },
          { street: 'ул. Вознесенского', dist: 'Северная проходная', price: 550, time: '50 мин', coords: [48.3410, 37.9870] },

          // Кондратьевка (650 ₽)
          { street: 'пос. Кондратьевка', dist: 'Пригород Горловки', price: 650, time: '60 мин', coords: [48.3350, 38.1500] },
          { street: 'ул. Остапенко', dist: 'Пригород Горловки', price: 650, time: '60 мин', coords: [48.3360, 38.1520] },
          ...
          { street: 'ст. Никитовка', dist: 'Северная проходная', price: 550, time: '50 мин', coords: [48.3560, 38.0130] },
          { street: 'пос. Широкая Балка', dist: 'Пригород Горловки', price: 650, time: '60 мин', coords: [48.2600, 37.9200] }
        ];
        ...
        function getZoneForCoords(coords) {
          if (!coords || isNaN(coords[0]) || isNaN(coords[1])) {
            return { dist: 'ЦГР', price: 250, time: '30 мин' };
          }
          var zones = window.GORLOVKA_DELIVERY_ZONES || [];
          for (var i = 0; i < zones.length; i++) {
            if (pointInPolygon(coords, zones[i].coords)) {
              return {
                dist: zones[i].name,
                price: zones[i].price,
                time: zones[i].time + ' мин'
              };
            }
          }
          return {
            dist: 'Пригород Горловки',
            price: 650,
            time: '60 мин'
          };
        }

        var activeSearchAbort = null;
        ...
        function initYandexMapGorlovka() {
          ...
          try {
            ymaps.ready(function() {
              ...
              function findNearestStreet(coords) {
                var minD = Infinity;
                var best = null;
                for (var i = 0; i < GORLOVKA_STREET_REGISTRY.length; i++) {
                  var it = GORLOVKA_STREET_REGISTRY[i];
                  if (it.coords) {
                    var d = Math.hypot(coords[0] - it.coords[0], coords[1] - it.coords[1]);
                    if (d < minD) {
                      minD = d;
                      best = it;
                    }
        ...
        if (btnApplyAddress) {
          btnApplyAddress.addEventListener('click', function() {
            ...
            var typedVal = mapAddressInput ? mapAddressInput.value.trim() : '';

            if (typedVal) {
              deliveryInfo.address = typedVal.indexOf('Горловка') !== -1 ? typedVal : ('Горловка, ' + typedVal);
              var localMatches = findLocalStreetMatches(typedVal);
              if (localMatches.length > 0) {
                deliveryInfo.district = localMatches[0].dist;
                deliveryInfo.price = localMatches[0].price;
                deliveryInfo.time = localMatches[0].time;
              } else if (window.checkCoordsZoneGlobal && window.gorlovkaClientPlacemark) {
                window.checkCoordsZoneGlobal(window.gorlovkaClientPlacemark.geometry.getCoordinates());
              }
              deliveryInfo.isAddressChosen = true;
        ...
        function updateCheckoutAddressUI() {
          ...
          var zoneMeta = document.getElementById('co-selected-zone-text');
          ...
          var isPickup = currentDeliveryMethod === 'pickup';
          var isAddressSelected = isPickup || (deliveryInfo && deliveryInfo.isAddressChosen && deliveryInfo.address);

          if (addrTitle) {
            if (isPickup) {
              addrTitle.textContent = 'г. Горловка, ул. Пушкинская, 36а';
              addrTitle.style.color = '#14241A';
            } else if (isAddressSelected) {
              addrTitle.textContent = deliveryInfo.address;
              addrTitle.style.color = '#14241A';
            } else {
              addrTitle.textContent = 'Адрес доставки не выбран (выберите на карте или введите)';
              addrTitle.style.color = '#6C8375';
            }
          }

          // Hide district, price and delivery time until address is selected
          if (zoneMeta) {
            zoneMeta.style.display = isAddressSelected ? 'flex' : 'none';
          }

          if (isAddressSelected) {
            if (zonePill) zonePill.textContent = isPickup ? 'Самовывоз: ЦГР' : ('Район: ' + (deliveryInfo.district || 'ЦГР'));
            if (pricePill) pricePill.textContent = 'Доставка: ' + (isPickup ? '0 ₽ (Бесплатно)' : (deliveryInfo.price || 250) + ' ₽');
            if (timePill) timePill.textContent = isPickup ? 'Ежедневно 08:00 – 20:00' : (deliveryInfo.time || '30-45 мин');
          }
        ...
        function updateCheckoutTotals() {
          ...
          var isPickup = currentDeliveryMethod === 'pickup';
          var isAddressSelected = isPickup || (deliveryInfo && deliveryInfo.isAddressChosen && deliveryInfo.address);
          var deliveryCost = isPickup ? 0 : (isAddressSelected ? (deliveryInfo.price || 250) : 0);
          ...
          if (deliveryEl) {
            if (isPickup) {
              deliveryEl.textContent = '0 ₽ (Бесплатно)';
              deliveryEl.style.color = '#2D6A4F';
            } else if (isAddressSelected) {
              deliveryEl.textContent = deliveryCost.toLocaleString('ru-RU') + ' ₽';
              deliveryEl.style.color = '#2D6A4F';
            } else {
              deliveryEl.textContent = '—';
              deliveryEl.style.color = '#6C8375';
            }
          }
        ...
        if (btnCoSubmitOrder) {
          btnCoSubmitOrder.addEventListener('click', function() {
            ...
            if (currentDeliveryMethod === 'courier' && (!deliveryInfo || !deliveryInfo.isAddressChosen || !deliveryInfo.address)) {
              showToast('Пожалуйста, выберите адрес доставки на карте или введите его');
              openAddressModal();
              return;
            }
        ...
        function navigateToView(viewName, categoryName, searchQuery) {
          ...
          if (viewName === 'catalog') {
            var alreadyInCatalog = catalogView && catalogView.style.display === 'block';
        ...
        function filterPageCatalog(categoryName, query) {
          ...
          pageCards.forEach(function(card) {
            ...
            if (cleanCat === 'all' || cleanCat === 'все товары' || cleanCat === 'весь каталог') {
            ...
            } else if (cleanCat.indexOf('моно') !== -1) {
              catMatch = cat.indexOf('моно') !== -1 || allCats.indexOf('моно') !== -1 || titleLower.indexOf('моно') !== -1 || titleLower.indexOf('роз') !== -1;
            } else if (cleanCat.indexOf('авторск') !== -1 || cleanCat.indexOf('размер m') !== -1) {
              catMatch = cat.indexOf('сборн') !== -1 || allCats.indexOf('размер m') !== -1 || allCats.indexOf('сборн') !== -1 || titleLower.indexOf('автор') !== -1;
            } else if (cleanCat.indexOf('классическ') !== -1 || cleanCat.indexOf('размер s') !== -1) {
              catMatch = allCats.indexOf('размер s') !== -1 || cat.indexOf('размер s') !== -1 || allCats.indexOf('классич') !== -1;
            } else if (cleanCat.indexOf('большие') !== -1 || cleanCat.indexOf('размер l') !== -1) {
              ...
              catMatch = cat.indexOf('свадеб') !== -1 || allCats.indexOf('свадеб') !== -1 || allCats.indexOf('невест') !== -1 || cat.indexOf('невест') !== -1;
            } else if (cleanCat.indexOf('ювелир') !== -1) {
              catMatch = cat.indexOf('ювелир') !== -1 || allCats.indexOf('ювелир') !== -1;
            } else if (cleanCat.indexOf('сухоцвет') !== -1) {
              catMatch = cat.indexOf('сухоцвет') !== -1 || allCats.indexOf('сухоцвет') !== -1;
            } else if (cleanCat.indexOf('сезон') !== -1) {
