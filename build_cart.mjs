    ...
    <script>
      (function() {
        ...
        function scrollTrackToCard(trackEl, direction) {
          ...
          var current = (typeof trackEl._targetScroll === 'number') ? trackEl._targetScroll : trackEl.scrollLeft;

          if (Math.abs(trackEl.scrollLeft - current) > (cards[0].offsetWidth * 1.5)) {
            current = trackEl.scrollLeft;
          }

          var step = cards[0].offsetWidth + 18;
          var target = current;

          if (direction > 0) {
            for (var i = 0; i < cards.length; i++) {
              var left = cards[i].offsetLeft - trackEl.offsetLeft;
              if (left > current + 15) {
                target = left;
                break;
              }
